	...
	<div class="login-container flx-center">
		<div class="login-box">
			<div class="login-form">
                ...
                <div>
                    <el-button :icon="Unlock" type="primary" @click="UnlockNew" circle />
                </div>
			</div>
		</div>
		<div class="bruce">
...
<script setup lang="ts" name='lockPage'>
    ...
    const UnlockNew = ()=>{
        ElMessageBox.prompt('请输入解锁密码', '', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            inputPattern:/\S+/,
            inputErrorMessage: '解锁密码不能为空',
        })
        .then(({ value }) => {
            if(value == store.state.lockForm.lockPassword){
                let form = {
                    isLcokView:false,
                    lockPassword:'',
                }
    ...
    watchEffect(()=>{
        setInterval(() => {
            currentYear.value= setDate(new Date())
            currentHour.value= setDate1(new Date())
        }, 1000);
    })
